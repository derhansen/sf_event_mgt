.. ==================================================
.. FOR YOUR INFORMATION
.. --------------------------------------------------
.. -*- coding: utf-8 -*- with BOM.

.. include:: /Includes.rst.txt


.. _known-problems:

==============
Known Problems
==============

* The extension does not (and will never) support recurring events. See :ref:`recurring-events` for more information.
* It is not possible to translate emails sent by the extension
* Prior to version 4.1.0 of the extension, the iCal download does not work, when
  :php:`$TYPO3_CONF_VARS['FE']['compressionLevel']` is set to a value > 0 - https://forge.typo3.org/issues/69223

Other known problems are listed on GitHub at https://github.com/derhansen/sf_event_mgt

If you find a bug or have a feature request for this extension,
please use the issuetracker on GitHub at https://github.com/derhansen/sf_event_mgt
